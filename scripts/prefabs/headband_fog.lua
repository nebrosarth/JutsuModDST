local assets =
{
	Asset("ANIM", "anim/headband_fog.zip"),
	Asset("ATLAS", "images/inventoryimages/headband_fog.xml"),
	
}
--- USEFUL FUNCTIONS ---

local function autoequip(headban, ninja)
	for key, sameband in pairs(ninja.components.inventory.itemslots) do
		if sameband.prefab == headban.prefab then
...
local function commonfn(anim, atlas)
	...
	minimap:SetIcon("headband_fog.tex")
...
local function headband_fog()
    local inst = commonfn("headband_fog", "headband_fog")
	
    return inst
end

return Prefab("headband_fog", headband_fog, assets)
